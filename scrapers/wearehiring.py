...
from scrapers.utils.common import headless_webdriver, salary_to_int
from scrapers.utils.wait import limit
...
HREF = "href"
# ****************** SCRAP CONSTANTS ******************
XPATH_NUM_OFFERS = '/html/body/div/form/section/div/div[1]/div[1]/span/b'
...
async def scrap_page(url: str, petition: Petition) -> UpdateResult:
    try:
        await limit()
        ...
        print(f"Scraping offer finished: {url}")

        return Offer.from_scrapped(job_dict, Source.WE_HIRING, petition.mongo_id).save()
    except Exception as e:
        logging.error(f"There was an unexecpected erro while scraping an offer in WeAreHiring: {e}")
...
async def scrap(petition: Petition):
    try:
        ...
        # Need to use firefox console to get info needed -> using execute_script
        ...
        for _ in range(int(num_offers.text.split()[0]) // 10):
            ...
            # Wait for page to extend
            await wait.loading()
            # get the height of new page
            page_height = driver.execute_script(SCROLL_HEIGHT_SCRIPT)

        page_source = driver.page_source
        driver.quit()
        # "This but a scratch. I've seen worst."-Black Knight, Monty Python and the Holy Grail
        ...
        print(f"There was an Unexpected error scraping the main page of we are hiring: {e}")

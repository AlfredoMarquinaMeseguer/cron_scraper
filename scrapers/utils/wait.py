...
async def loading(min_w: int = 5, max_w: int = 7) -> None:
    """
    ...
    """
    await asyncio.sleep(randint(min_w, max_w))


async def limit(min_w: int | float = 0.5, max_w: int | float = 1.5) -> None:
    """
    ...
    """
    await asyncio.sleep(random() / (max_w - min_w + 1) + min_w)

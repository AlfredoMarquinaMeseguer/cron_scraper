...
from scrapers.utils.common import round_up, salary_to_int
...
async def scrap(petition: Petition) -> list[dict[str, Any]]:
    ...
    try:
        ...
        print(f"There was an unexpected error scraping main tecnoempleo page: {e}")
...
async def scrap_page(url: str, petition_id) -> None:
    try:
        ...
        return_dictionary[offer_consts.DESCRIPTION] = soup.find("div", attrs={"itemprop": "description"}).get_text(). \
            strip()
        return_dictionary[offer_consts.ID] = return_dictionary.get(offer_consts.LINK_JOB).split("/")[-1]

        logging.info(f"Offer scraped from tecno-empleo: {url}")
        print(f"Offer scraped from tecno-empleo: {url}")
        return Offer.from_scrapped(return_dictionary, Source.TECNO_EMPLEO, petition_id).save()
    except Exception as e:
        logging.error(f"There has been an unexpected error while scraping a tecnoempleo offer {url}: {e}")
